...
extern bit			FP_Rcv_timeOut_FLG;		//指纹模块数据接收超时标志	
...
#define S2RI  0x01
	...
	while(!RI2){
		...
		if(!tOut_cnt){	//超时检测
			...
			LogString("指纹模块接收响应数据超时!!!\n");
			FP_Rcv_timeOut_FLG = 1;
			CLR_RI2();
			return 0xff;
		}
	}
...
/********************* UART2中断函数************************/
void UART2_Rountine (void) interrupt UART2_VECTOR
{
//	if(RI2)
//	{
//		CLR_RI2();
